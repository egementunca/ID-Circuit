...
def create_parser() -> argparse.ArgumentParser:
    ...
    gen_parser.add_argument('gate_count', type=int, help='Number of gates in forward circuit')
    gen_parser.add_argument('--max-inverse-gates', type=int, default=40,
                           help='Maximum gates in inverse circuit (default: 40)')
    gen_parser.add_argument('--no-unroll', action='store_true',
                           help='Skip unrolling step')
    gen_parser.add_argument('--no-post-process', action='store_true',
                           help='Skip post-processing step')
    gen_parser.add_argument('--solver', default='minisat-gh',
                           help='SAT solver to use (default: minisat-gh)')
    gen_parser.add_argument('--output', '-o', help='Output file for results')
    
    # List command
    list_parser = subparsers.add_parser('list', help='List dimension groups and circuits')
    ...
    list_parser.add_argument('--show-representatives', action='store_true',
                           help='Show representative circuits')
    list_parser.add_argument('--show-equivalents', action='store_true',
                           help='Show equivalent circuits count')
    list_parser.add_argument('--format', choices=['table', 'json'], default='table',
                           help='Output format')
    
    # Unroll command
    unroll_parser = subparsers.add_parser('unroll', help='Unroll dimension groups')
    unroll_parser.add_argument('dim_group_id', type=int, nargs='?',
                             help='Dimension group ID to unroll (optional)')
    unroll_parser.add_argument('--all', action='store_true',
                             help='Unroll all unprocessed dimension groups')
    unroll_parser.add_argument('--types', nargs='+',
                             choices=['swap', 'rotation', 'permutation', 'reverse', 'local_unroll', 'full_unroll'],
    ...
    batch_parser.add_argument('--max-inverse-gates', type=int, default=40,
                            help='Maximum gates in inverse circuit')
    batch_parser.add_argument('--output-dir', help='Output directory for results')
    
    return parser

def format_dimension_groups(dim_groups: List, show_representatives: bool = False, 
                          show_equivalents: bool = False, format_type: str = 'table') -> str:
    ...
    lines.append("=" * 80)
    lines.append(f"{'ID':<5} {'Width':<6} {'Gates':<6} {'Circuits':<10} {'Processed':<10}")
    lines.append("-" * 80)
    
    for dg in dim_groups:
        lines.append(f"{dg.id:<5} {dg.width:<6} {dg.gate_count:<6} {dg.circuit_count:<10} {'Yes' if dg.is_processed else 'No':<10}")
...
def cmd_generate(args, factory: IdentityFactory) -> int:
    ...
    try:
        ...
        if result['success']:
            print(f"✓ Successfully generated identity circuit ({args.width}, {args.gate_count})")
            ...
            if 'seed_generation' in result:
                ...
                if hasattr(seed_result, 'representative_id') and seed_result.representative_id:
...
def cmd_list(args, factory: IdentityFactory) -> int:
    ...
    try:
        ...
        output = format_dimension_groups(
            dim_groups, 
            args.show_representatives, 
            args.show_equivalents, 
            args.format
        )
        print(output)
            ...
            for dg in dim_groups:
                ...
                if representatives:
                    ...
                    for rep in representatives:
                        ...
                        print(f"  Rep {rep.id}: Circuit {rep.circuit_id}, Composition: {rep.gate_composition}{primary_str}")
    ...
    try:
        if args.all:
            ...
            for dim_group_id, result in results.items():
                if result.success:
                    print(f"  Group {dim_group_id}: {result.total_equivalents} equivalents")
                else:
            ...
            result = factory.unroller.unroll_dimension_group(args.dim_group_id, args.types)
...
def cmd_analyze(args, factory: IdentityFactory) -> int:
    ...
    try:
        ...
        for rep in analysis['representatives']:
            primary_str = " (PRIMARY)" if rep['is_primary'] else ""
            print(f"  {rep['id']}: Circuit {rep['circuit_id']}, Composition: {rep['gate_composition']}{primary_str}")
